...
def calculate(
    ...
    return calculate_missouri_income_tax(owners, Decimal(federal_tax), Decimal(conversions), rules)
...
@pytest.mark.parametrize("field", ["private_pension", "taxable_rmd"])
...
def test_roth_conversion_fails_explicitly(rules: MissouriTaxRules) -> None:
    with pytest.raises(ValueError, match="Roth-conversion subtraction treatment is unsupported"):
        calculate(rules, (owner(),), conversions="10000")
